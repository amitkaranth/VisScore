...
def _run_generation(
    ...
    for i, uf in enumerate(uploaded_files):
        ...
        try:
            ...
            manifest.append({"source_csv": str(in_path), "status": "error", "error": str(e), "charts": []})

    (out_root / "csv_tufte_manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
...
def main() -> None:
    ...
    if manifest_path.is_file():
        st.download_button("Download manifest (JSON)", data=manifest_path.read_bytes(), file_name="csv_tufte_manifest.json", mime="application/json")
    zip_bytes = _manifest_to_zip_bytes(out_root)
    st.download_button("Download all PNGs + manifest (ZIP)", data=zip_bytes, file_name="csv_tufte_charts_export.zip", mime="application/zip")
                    ...
                    with c1:
                        ...
                        for c in row_bad:
                            ...
                            if p.is_file():
                                ...
                                cap = rel + (f" — {', '.join(viol[:3])}…" if len(viol) > 3 else (f" — {', '.join(viol)}" if viol else ""))
