...
This is migrated from the repo-root `vlm_judge.py` so it can be imported cleanly
from Streamlit and CLI entrypoints.
"""
...
def judge_chart_gemini(image_path: str, api_key: str | None = None, model_name: str | None = None) -> dict[str, Any]:
    ...
    for mid in candidates:
        try:
            ...
            if "404" in err or "not found" in err.lower() or "429" in err or "quota" in err.lower():
                continue
            return {"verdict": None, "reasoning": "", "model": mid, "error": f"Gemini API error: {e}"}
        ...
        try:
            ...
            if not reasoning:
                reasoning = text[:2000]
            return {"verdict": verdict, "reasoning": reasoning, "model": used_model, "raw_text": text}
...
def judge_chart_groq(image_path: str, api_key: str | None = None, model_name: str | None = None) -> dict[str, Any]:
    key = api_key or os.environ.get("GROQ_API_KEY")
    if not key:
        return {"verdict": None, "reasoning": "", "model": None, "error": "Missing GROQ_API_KEY. Free key: https://console.groq.com/keys"}
    ...
    try:
        ...
        return {"verdict": None, "reasoning": "", "model": candidates[0], "error": f"Could not prepare image for Groq: {e}"}
        ...
        try:
            ...
            if not r.ok:
                ...
                if r.status_code in (400, 404):
                    continue
                return {"verdict": None, "reasoning": "", "model": mid, "error": f"Groq API error: {last_err}"}
            body = r.json()
            text = (body.get("choices") or [{}])[0].get("message", {}).get("content") or ""
            used_mid = mid
            break
        except requests.RequestException as e:
            return {"verdict": None, "reasoning": "", "model": mid, "error": f"Groq API error: {e}"}

    if not used_mid:
        return {"verdict": None, "reasoning": "", "model": candidates[0] if candidates else None, "error": f"Groq: all model attempts failed. Last: {last_err}"}

    text = text.strip()
    if not text:
        return {"verdict": None, "reasoning": "", "model": used_mid, "error": "Empty response from Groq."}
...
def combine_cnn_and_vlm(cnn_label: str, vlm_result: dict[str, Any]) -> dict[str, Any]:
    ...
    if cnn == vlm:
        return {"final_verdict": cnn, "agreement": "unanimous", "cnn_vote": cnn, "vlm_vote": vlm}
